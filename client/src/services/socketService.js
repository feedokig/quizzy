...
class SocketService {
  ...
  setupListeners() {
    ...
    this.socket.on("player-answered", (data) => {
      if (this.callbacks.onPlayerAnswered)
        this.callbacks.onPlayerAnswered(data);
    });

    this.socket.on("update-players", (players) => {
      if (this.callbacks.onUpdatePlayers)
        this.callbacks.onUpdatePlayers(players);
    });
    ...
    this.socket.on("show-answer-history", (answerData) => {
      ...
      if (this.callbacks.onShowAnswerHistory)
        this.callbacks.onShowAnswerHistory(answerData);
    });

    this.socket.on("fifty-fifty-options", (options) => {
      if (this.callbacks.onFiftyFiftyOptions)
        this.callbacks.onFiftyFiftyOptions(options);
    });

    this.socket.on("boost-activated", (data) => {
      if (this.callbacks.onBoostActivated)
        this.callbacks.onBoostActivated(data);
    });
    ...
    this.socket.on("max-players-updated", (data) => {
      if (this.callbacks.onMaxPlayersUpdated)
        this.callbacks.onMaxPlayersUpdated(data);
    });
  ...
  updateMaxPlayers(pin, maxPlayers) {
    ...
    console.log(
      `Emitting update-max-players with pin ${pin} and maxPlayers ${maxPlayers}`
    );
  }
